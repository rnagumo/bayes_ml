...
def plot_reconstruction(x, q, p, dim_1, dim_2):
    with torch.no_grad():
        z = q.sample({"x": x}, return_all=False)
        recon_batch = p.sample_mean(z).view(-1, 1, dim_1, dim_2)
        comparison = torch.cat(
            [x.view(-1, 1, dim_1, dim_2), recon_batch]).cpu()

        return comparison


def plot_image_from_latent(z_sample, p, dim_1, dim_2):
    with torch.no_grad():
        sample = p.sample_mean({"z": z_sample}).view(-1, 1, dim_1, dim_2).cpu()
